...
function mdSelect($compile, $parse) {
  ...
  function postLink(scope, element, attrs, ctrls) {
    ...
    function autoSelectXs() {
      return attrs.mdAutoSelectXs === '' || self.autoSelectXs;
    }
    ...
    function mdDataTableAutoSelectService() {
      var autoSelectShow = false;
      if (parseInt(window.innerWidth)<=600) {
        autoSelectShow = true;
      }
      return autoSelectShow;
    }
    scope.$watch(autoSelectXs, function (newValue, oldValue) {
      var teste1 = false;
      if (newValue || newValue===undefined) {
        ...
        function watchResize () {
          teste1 = mdDataTableAutoSelectService();
          if (teste1) {
            // element.prepend(createCheckbox());
            element.on('click', toggle);
          } else {
